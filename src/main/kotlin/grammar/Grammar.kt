...
data class SpecialNonTerminal(val name: String, val filter: (Char) -> Boolean) : Symbol() {
  override fun toString(): String = name
  operator fun not(): SpecialNonTerminal = SpecialNonTerminal(name, { !filter(it) })
  operator fun plus(other: SpecialNonTerminal): SpecialNonTerminal =
    SpecialNonTerminal(name + other.name, { filter(it) || other.filter(it) })
}
